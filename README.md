...
# 파일을 압축,해제 tar
````
tar cvfz [압축파일이름].tar.gz [압축할 파일 경로들]
ex) tar cvfz test.tar.gz ./gigi ./gkgk
ex) tar cvfz test.tar.gz * :현재 위치한 모든 파일들 압축시
tar라는 명령어로 gz으로 압축했다는 뜻 cvf파일을 한묶음으로 z를 추가해 압축했다는 의미
...
# less 파일을 읽기,출력,확인
````
파일의 내용을 확인할때 less를 사용하면 읽는 부분만을 메모리에 올리기때문에 자원낭비를 막을수있다 원격으로 이용해야하는 상황에서 큰파일을 읽을시 부담이 클수있다.
less [파일명]

````

# cat 파일 읽기,내용 복사 전달,복제
````
cat [이름].txt : 파일의 내용을 출력해준다.

cat test01.txt > test02.txt : test01의 내용을 test02를 만들면서 복사해준다

cat test01.txt >> test02.txt : 01의 모든내용을 옮겨서 02를 초기화하지않고 02마지막 부분에 내용을 추가해준다.
````
